...
class LoginCreator:
  ...
  def create(self, username:str, password:str)-> dict:
    ...
    self.__validate_password(password=password, hashed_password=hashed_password)
  ...
  def __find_user(self, username:str)-> tuple[int, str, str]:
    ...
    if not user: raise Exception("Usuário nao encontrado")
    return user
  ...
  def __validate_password(self, password:str, hashed_password:str)-> None:
    ...
    if not is_password_correct : raise Exception("Senha incorreta!")
